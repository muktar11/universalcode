...
import { useUpdatePost, useDeletePost } from "@/lib/react-query/queries";
import { IUpdatePost, IDeletedPost } from "@/types";
  ...
  // Handler
    ...
    const submitForm = async (value: z.infer<typeof PostValidation>) => {
      try {
        ...
        console.log(error);
        toast({
          title: `${action} An error occurred while updating event. Please try again`,
        });
      }
    };


    const { mutateAsync: deletePostAsync } = useDeletePost(post._id);

    const deleteForm = async () => {
      try {
        const postToDelete: IDeletedPost = {
          _id: post?._id || "",
        };
...
<div className="flex-center gap-2 text-light-3">
  ...
  <Button type="button" size="sm" className="shad-button_primary px-5" onClick={deleteForm}>Delete</Button>
</div>
{showFields && (
