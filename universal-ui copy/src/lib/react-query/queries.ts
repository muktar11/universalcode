...
export const useDeletePost = (_id: string) => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (_id: string) => DeletePost(_id), // Pass postId to DeletePost
    onSuccess: (_, _id: string) => {
      queryClient.invalidateQueries({
        queryKey: [QUERY_KEYS.GET_UPDATE_POST, _id], // Pass postId to queryKey
      });
    },
  });
};
...
export const useUpdateCourse = (_id: string) => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (course: any) => UpdateCourse(course, _id),
    onSuccess: () => {
      queryClient.invalidateQueries({
...
export const useDeleteCourse = (_id: string) => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (_id: string) => DeleteCourse(_id), // Pass postId to DeletePost
    onSuccess: (_, _id: string) => {
      queryClient.invalidateQueries({
        queryKey: [QUERY_KEYS.GET_UPDATE_POST, _id], // Pass postId to queryKey
      });
...
export const useUpdateEvent = (_id: string) => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (event: any) => updateEvent(event, _id),
    onSuccess: () => { // Adding type guard to ensure event is defined
        queryClient.invalidateQueries({
          queryKey: [QUERY_KEYS.GET_UPDATE_COURSE, _id],
        })
    },
...
export const useDeleteEvent = (_id: string) => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (_id: string) => DeleteEvent(_id), // Pass postId to DeletePost
    onSuccess: (_, _id: string) => {
      queryClient.invalidateQueries({
        queryKey: [QUERY_KEYS.GET_UPDATE_POST, _id], // Pass postId to queryKey
      });
