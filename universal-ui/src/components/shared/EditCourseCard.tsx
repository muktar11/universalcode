...
const EditCourseCard = ({ course }: CourseCardProps) => {
  const { mutate, isLoading } = useUpdateCourse(course._id); // Pass course._id to the usePurchseCourse hook
  const navigate = useNavigate();
  const handleEdit = async () => {
    try {
      await mutate(); // Call the mutation function when the button is clicked
      navigate(`/edit-course/${course._id}`) // Redirect to "/Profile" after successful purchase
    } catch (error) {
